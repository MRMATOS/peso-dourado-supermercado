        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={productId || ''} onValueChange={value => setProductId(value || null)} disabled={itemType !== 'Osso'}>
                <SelectTrigger id="productId" className="rounded-lg h-9">
                  <SelectValue placeholder="Selecione o produto" />
                </SelectTrigger>
                <SelectContent>
                  {filteredProducts.map(product => <SelectItem key={product.id} value={product.id}>
                      {product.description}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <NumberInput id="grossWeight" value={grossWeightKg} onChange={setGrossWeightKg} decimalPlaces={2} placeholder="0,00" className="input-weight" suffix="kg" clearOnFocus ref={grossWeightInputRef} onKeyDown={handleKeyDown} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <NumberInput id="tare" value={tareKg} onChange={setTareKg} decimalPlaces={2} placeholder="0,00" suffix="kg" disabled={!itemType} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="unitPrice">Preço por kg</Label>
              <NumberInput id="unitPrice" value={unitPrice} onChange={setUnitPrice} decimalPlaces={2} prefix="R$" disabled={!itemType} className={!itemType ? "bg-gray-50" : ""} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="netWeight">Peso Líquido (kg)</Label>
              <NumberInput id="netWeight" value={netWeightKg} onChange={() => {}} // Read-only
            decimalPlaces={2} suffix="kg" disabled className="bg-gray-50" />
            </div>
          </div>
          ...
          <div className="flex items-end gap-4 pt-2">
            <div className="w-1/4 md:w-1/6">
              ...
              <NumberInput id="totalPrice" value={totalPrice} onChange={() => {}} // Read-only
            decimalPlaces={2} prefix="R$" disabled className="bg-gray-50 font-bold text-emerald-600" />
            </div>
            <div className="flex-1 flex justify-end">
              <Button type="submit" className="px-6 whitespace-nowrap" size="sm" disabled={!itemType || grossWeightKg <= 0 || itemType === 'Osso' && !productId}>
                Adicionar
              </Button>
            </div>
          </div>
